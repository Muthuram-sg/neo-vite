...
const useOEEAssetStatus = () => {
  ...
  const geAssetStatus = async (schema, data, start_date, end_date,cycle_time,partData) => {
    setLoading(true);
    ...
    let isLongRange = ((new Date(end_date) - new Date(start_date)) / 60000) > 1440 ? true : false
          ...
          if (response && !response.errorTitle) {
            ...
            const tempFunc =(data)=>{
              data.forEach((x, i) => {
                ...
                // console.log(x.next,x.time,"Activeeee",moment(x.next).format('YYYY-MM-DD HH:mm:ss'),moment(x.time).format('YYYY-MM-DD HH:mm:ss'),moment(x.next).valueOf(),i,diff,val.mic_stop_duration)
                ...
                if (!val.is_status_signal_available) {
                  ...
                  if (i === (data.length - 1)) {
                    ...
                    let Lastpart = partData ? partData[0].data.filter(e=> moment(new Date(e.time)).isBetween(moment(Time),moment(end_date))) : [] 
                ...
                final_result.forEach(x => {
                  ...
                  if (!val.is_status_signal_available) {
                    ...
                    if (x.reason && x.include_in_oee) {
                      ...
                      let hoursDiff1 = (endhour1 - starthour1) / 3600000;
                    ...
                    if (x.value !== 'ACTIVE' && x.reason && x.include_in_oee) {
                      ...
                      let hoursDiff2 = (endhour - starthour) / 3600000;
                      // checking downtime lies between same hour
                      let HHformat = isLongRange ? "00" : "HH"
            ...
            if (response.data.length > 0) {
                ...
                console.log(response,"responseresponseCore")
                if(!val.is_status_signal_available){ 
                  if(moment(response.data[0].time)>moment(start_date)){ 
                    ...
                    if(diff > val.mic_stop_duration && partData && partData[0].data.length>0){ 
                      
                      let Lastpart =partData[0].data.filter(e=> moment(new Date(e.time)).isBetween(moment(start_date),moment(next))) 
                      next =Lastpart.length > 0 ? Lastpart[Lastpart.length-1].time : next
                      time = start_date
                      arr1 = [
                        {
                  ...
                  let Lastpart =partData ? partData[0].data.filter(e=> moment(new Date(e.time)).isBetween(moment(response.data[response.data.length-1].time),moment(end_date))) : []
                  let Lastdiff = moment.duration(moment(end_date).diff(moment(Lastpart.length > 0 ? Lastpart[0].time : response.data[response.data.length-1].time))).asSeconds()
                  // console.log(Lastdiff,"LastdiffLastdiff",response.data[response.data.length-1],Lastpart)
                ...
                tempFunc(response.data) 
                getOutages(response.data) 
                // console.log(final_outages,"final_outages")
                if (final_outages.length > 0) {
                    getFinal_result() 
                }
              // console.log({ raw: final_result, data: temp, dTime: dTime, totalDTime: totalDTime, reasons: reasons, status: checkStatus,hourlyDowntime:houryDowntime },"13th")
              return { raw: final_result, data: temp, dTime: dTime, totalDTime: totalDTime, reasons: reasons, status: checkStatus,hourlyDowntime:houryDowntime }
            } else {
                  ...
                  let Lastarr = [
                    {
                      ...
                      time: moment(partData[0].data[0].time).add(cycle_time ? Number(cycle_time) : val.mic_stop_duration,'second').format('YYYY-MM-DDTHH:mm:ssZ'),
                      next: end_date,
                  ...
                  let firstarr = [
                    {
                      ...
                      key: partData[0].data[0].key,
                      time: start_date,
                      next: moment(partData[0].data[partData[0].data.length - 1].time).subtract(cycle_time ? Number(cycle_time) : val.mic_stop_duration,'second').format('YYYY-MM-DDTHH:mm:ssZ'),
                      value: partData[0].data[0].value ? 0 : 1 
                    }
                  ]
                  arr1 = [...firstarr,...arr1]
            ...
            return { raw: [], data: response, dTime: 0, totalDTime: 0, status: 'IDLE',hourlyDowntime: {}}
          }
        })
        .catch((e) => {
...
export default useOEEAssetStatus;