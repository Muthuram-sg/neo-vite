...
import {
    currentUserRole, ExecDialog, Loadingpanel, themeMode, userData, userLine, selectedPlant, oeeProdData, showOEEAsset, snackToggle, snackMessage, snackType,  customdates,QualityMetrics, ProgressLoad, ProdBtnGrp, oeeAssets, executionData, executionDetails,adddtreasondisbale, pickerDisable,DateSrch
} from "recoilStore/atoms";
import { useMutation } from "@apollo/client";
 
import OEECard from './ProductionChild/OEECard' 
import PartCard from './ProductionChild/PartCard'
import DowntimeCard from './ProductionChild/DowntimeCard'
import DowntimeModalDialog from './ProductionChild/DowntimeModal';
...
function DownCommentModel(props){
    const downtimeModalRef = useRef();

    useEffect(()=>{
        // console.log(props.reasonModelData,"props.reasonModelData")
        if(props.reasonModelData){
            setTimeout(()=>{
                        downtimeModalRef.current.openReasonDialog(props.reasonModelData)
            },200)
        }
    },[props.reasonModelData])
    ...
    return (
        <ModalNDL onClose={props.handleReasonDialogClose} maxWidth={"xs"} open={props.reasonDialog}>
            <DowntimeModalDialog
            handleReasonDialogClose={props.handleReasonDialogClose}
            reasonDialog={props.reasonDialog}
            ref={downtimeModalRef}
            entity_id={props.entity_id}
            userid={props.userid}
            headplantid={props.headplantid}
            treiggerOEE={props.treiggerOEE}
            outGRData={props.outGRData}
            ReasonTagsListData={props.ReasonTagsListData} 
            reasonTypeData={props.reasonTypeData}
            />
        </ModalNDL> 
    )
}
...
export default function ProductionDashboard() {
    ...
    const [savedQualityMetrics] = useRecoilState(QualityMetrics)
    ...
    const [Timeout, setRangeTimeout] = useState(20000);
    ...
    const [assertStatusData,setassertStatusData] = useState([]) 
    const {  ReasonTagsListData, getReasonTags } = useReasonTags()
    const {  outGRData, getReasons } = useReasons(); 
    const [reasonModelData,setreasonData] = useState('')
    // const downtimeModalRef = useRef();
    ...
    useEffect(()=>{
        // console.log(Timeout,"Timeout",reasonDialog)
        if(workExecutionDetails.some(f=> !f?.ended_at) && (ProdGroupRange === 6 || ProdGroupRange === 11 ) && !reasonDialog){
            setRangeTimeout(20000)
        }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [partData,reasonDialog])

    useEffect(() => { 
        getWorkOrderLine(headPlant.id)
        getReasons()
        getReasonTags(headPlant.id)
        setRangeTimeout(null)
        setOEEConfigData([])
        setExecutionList([])
    ...
    useEffect(() => {
        ...
        if (selectedAssetID.id) {
            // setProgressBar(true)
            setTimeout(()=>{
                // console.log(selectedAssetID,"selectedAssetID",Timeout,reasonDialog)
        ...
        if ((!cycleError && !cycleLoading && cycleData) && (!partError && !partLoading && partData) && (!partSumError && !partSumLoading && partSumData) && (!qualDefError && !qualDefLoading && qualDefData)) {
            ...
            let oee = configParam.OEE_PROD_VALUE(
                ...
                partData && partData[0] && partData[0][0].assetStatData && partData[0][0].assetStatData.hourlyDowntime
                    ...
                    : 0,
                partData && partData[0] && partData[0][0].assetStatData && partData[0][0].assetStatData.onlyplaneddt !== undefined ? partData[0][0].assetStatData.onlyplaneddt : 0,


            );
            // console.log("Final_result.dTime",oee)
    ...
    useEffect(()=>{ 
        // console.log(isLongRange,"isLongRange",startRange,endRange)
        if(oeeConfigData.length>0 && startRange){
            if(isDryer){
                ...
                triggerprodOEE(workExecutionDetails,startRange,endRange,false,isLongRange)
    ...
    useInterval(() => {
        ...
        // console.log(Timeout,"Timeout2",reasonDialog)
        let endrange = moment(new Date()).format("YYYY-MM-DDTHH:mm:ss"+ TZone)
        if(Timeout && !reasonDialog){
    ...
    async function triggerprodOEE(data,startrange,endrange,isRepeat,isLong){
      ...
      setStartRange(start);
      setEndRange(end)
        // console.log(val,"triggerprodOEE",start,end,isLong)
        getPartsCompleted(headPlant.schema, oeeConfigData, start, end,isRepeat,partData && partData.length>0 && partData[0] && partData[0][0].data?partData[0][0].data:[],(workExecutionDetails) ? workExecutionDetails : [],partData && partData.length>0 && partData[0] ? partData[0][0].cycleTime : 0,'','','',isLong)
    ...
    async function triggerOEE(data, isRepeat,dialog) {
        let startrange = moment(customdatesval.StartDate).format("YYYY-MM-DDTHH:mm:ss"+ TZone)
        let endrange = moment((isRepeat || dialog) ? new Date() : customdatesval.EndDate).format("YYYY-MM-DDTHH:mm:ss"+ TZone) 
        ...
        // console.log(startrange,"startrange",endrange,ProdGroupRange,totalDays,isRepeat)
            ...
            if(isDryer){  
                ...
                if (shiftAvailable && val) { 
                    if(!isRepeat){
                        ...
                        if(totalDays > 1440){
                            triggerprodOEE(workExecutionDetails,startrange,endrange,isRepeat,true)  
                        }else{
                            triggerprodOEE(workExecutionDetails,startrange,endrange,isRepeat,false)  
                        }
    ...
    useEffect(()=>{
        ...
        if(isDryer){
            if(dryerExecData && dryerExecData[0] && dryerExecData[0].dTime >= 0){
                setassertStatusData(dryerExecData[0]) 
            }else{
                setassertStatusData([])
            }
        }else{
          if(partData && partData[0] && partData[0][0].assetStatData && partData[0][0].assetStatData.dTime >= 0){
            // console.log(partData,"assertStatusData",partData[0][0].assetStatData)
            setassertStatusData(partData[0][0].assetStatData)  
          }else{
            setassertStatusData([])
          }  
        }
    },[isDryer,partData,dryerExecData])
    ...
    // console.log(partData,"partDatapartData")
                ...
                <Grid container spacing={3}> 
                        ...
                        (
                            ...
                            <React.Fragment>
                                ...
                                {!partError ?
                                    <Grid item xs={12} >
                                        <AssetStatusCard loading={partLoading} start={startRange} end={endRange} assetStatusData={partData && partData[0] && partData[0][0].assetStatData && partData[0][0].assetStatData.dTime >= 0 ? partData[0][0].assetStatData : [] } OEEData={oEE} downtimeReason={!outDTLoading && !outDTError && outDTData ? outDTData : []} />
                                    </Grid> : ""}
                                                ...
                                                {isDryer ? (
                                                    ...
                                                    <React.Fragment>
                                                        {/* <Grid item xs={12} sm={3} >
                                                            <ProductionRate loading={(!cycleLoading && !partLoading && !partSumLoading && !assetStatLoading && !qualDefLoading) ? false : true} OEEData={oEE} isDryer={isDryer} />
                                                        </Grid> */}
                                                        <Grid item xs={12} sm={4} >
                                                             <DowntimeCardForLongRange
                                                                ...
                                                                endDate={endRange}
                                                                loading={partLoading}
                                                                assetRawStatus={partData && partData[0] && partData[0][0].assetStatData && partData[0][0].assetStatData.dTime ? partData[0][0].assetStatData : [] }
                                        ...
                                        <Grid item sm={12}>
                                                ...
                                                <Grid item xs={12} lg={12}>
                                            {
                                                isDryer ?
                                                    ...
                                                    :
                                                    <PartsPerHourCard loading={partLoading} ProdGroupRange={ProdGroupRange} partLoading={partSumLoading} assetStatusData={partData && partData[0] && partData[0][0].assetStatData && partData[0][0].assetStatData.hourlyDowntime ? partData[0][0].assetStatData.hourlyDowntime : [] } dressLoading={dressingCountLoading} partsData={partData && (partData.length>0) && partData[0] && partData[0][0].data ? partData[0][0].data : [] } headPlant={headPlant} downtimeReason={!outDTLoading && !outDTError && outDTData ? outDTData : []} trendsData={PerHourData} triggerDressParts={getDressingParts} dressingCount={dressingCountData} DressData={PerDressData} OEEData={OEEData} expectedCycleTime={oEE.expCycle ? oEE.expCycle : 0} actualCycleTime={oEE.actcycle ? oEE.actcycle : 0} isLongRange={isLongRange}
                                            ...
                                            <Grid container spacing={4} >
                                                ...
                                                {isDryer ? (
                                                    ...
                                                    <React.Fragment>
                                                        ...
                                                        <Grid item xs={12} sm={6}  >
                                                            <DowntimeCard
                                                                loading={(partLoading || WorkExecutionLoading) ? true : false }
                                                                oeeConfigData={oeeConfigData}
                                                                prodReasonType={!reasonTypeLoading && !reasonTypeError && reasonTypeData}
                                                                reasons={!reasonLoading && !reasonError && reasonData}
                                                                assetRawStatus={
                                                                    ProdGroupRange === 6 || ProdGroupRange === 19 || ProdGroupRange === 11?
                                                                        partData && partData[0] && partData[0][0].assetStatData ? partData[0][0].assetStatData : []
                                                                        
                                                                   :
                                                                        !partLoading && partData && partData[0] && partData[0][0].assetStatData ?
                                                                            partData[0][0].assetStatData: []
                                                                ...
                                                                treiggerOEE={() => triggerprodOEE(workExecutionDetails, startRange, endRange,false)}
                                                                partsData={partData && (partData.length>0) && partData[0] && partData[0][0].data ? partData[0][0].data : [] }
                                                                ProdGroupRange={ProdGroupRange}
                                                                workExecutionDetails={workExecutionDetails}
                                                                Partloading={partLoading}
                                                                openReasonDialog={openDialogReason}
                                        ...
                                        <Grid item sm={!isDryer ? 4 : 12} >
                                            {
                                                !isDryer ? 
                                                    <PartCard
                                                        loading={partLoading}
                                                        oEE={oEE}
                                                        assetStatusData={(partData && partData[0] && partData[0][0].assetStatData && partData[0][0].assetStatData.dTime >= 0) ? partData[0][0].assetStatData : [] }
                                                        prodReasonType={!reasonTypeLoading && !reasonTypeError && reasonTypeData}
                                                        partsData={(partData && partData.length>0 && partData[0] && oEE.expCycle !== 0 && partData[0][0].data )? partData[0][0].data : []}
                                                        currUserRole={currUserRole}
                                                        entity_id={OEEData.entity_id}
                                                        userid={currUser.id}
                                                        headplantid={headPlant.id}
                                                        parameters={savedQualityMetrics}
                                                        IntrType={IntrType}
                                                        ProdGroupRange={ProdGroupRange}
                                                        assetType={OEEData && OEEData.entity && OEEData.entity.asset_types}
                                                         SteelForm={OEEData && OEEData.entity && OEEData.entity.steel_asset_configs}
                                                        configData = {oeeConfigData[0]}
                                                        treiggerOEE={() => triggerprodOEE(workExecutionDetails, startRange, endRange,false)}
                                                    />
                                                    :
                                                    <EfficiencyCard   efficiencyLable={efficiencyLable} isLongRange={isLongRange} shiftLoading={ShiftEfficiencyLoading} hourLoading={HourlyEfficiencyLoading} start={dryerStart} end={dryerEnd} shiftData={dryerShiftData} hourlyData={dryerHourlyData} />
...
<Grid container spacing={1} style={{ padding: 12 }}>
    ...
    const openDialogReason=(value)=>{
        // alert("openReasonDialog")
        setRangeTimeout(null)
        try {            
    ...
    return (
        ...
        <div>
            ...
            outGRData={outGRData}
            ReasonTagsListData={ReasonTagsListData} 
            reasonModelData={reasonModelData}
            reasonTypeData={!reasonTypeLoading && !reasonTypeError && reasonTypeData}
            />
