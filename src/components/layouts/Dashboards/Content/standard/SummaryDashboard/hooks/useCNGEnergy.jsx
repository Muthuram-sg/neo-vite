...
const useCNGEnergy = () => {
    const [CNGEnergyLoading, setLoading] = useState(false);
    const [CNGEnergyData, setData] = useState(null);
    const [CNGEnergyError, setError] = useState(null);
    const [customdatesval] = useRecoilState(customdates);

    const getCNGEnergy  = async (childLine,btGroupValue,userDefaultList,typelist,headPlant ) => {
        setLoading(true);
        ...
        Promise.all(
            childLine.filter(e=> e.line_id !== headPlant.id).map(async (val,i) =>{
                ...
                if (childPlant.node && childPlant.node.nodes && (childPlant.node.nodes.filter(f=> f.type === 4).length>0)) {
                    let type = childPlant.node.nodes.filter(f=> f.type === 4)[0].radio
                    let HierarchyData = []
                    if(type === 'hierarchy'){
                        HierarchyData = await configParam.RUN_GQL_API(gqlQueries.GetLineHierarchy, { line_id: val.line_id },val.token,val.line_id)
                        .then((lineHierData) => {
                            if (lineHierData !== undefined && lineHierData.neo_skeleton_hierarchy && lineHierData.neo_skeleton_hierarchy.length > 0) {
                                return lineHierData.neo_skeleton_hierarchy
                            } else {
                                return []
                            }
                        });
                    ...
                    if (type === 'hierarchy' && childPlant.node.nodes.filter(f=> f.type === 4)[0].selectedHierarchy && HierarchyData.length > 0) {
                        selecetedHierarchy = HierarchyData.find(x => x.id === childPlant.node.nodes.filter(f=> f.type === 4)[0].selectedHierarchy)
                        ...
                        // console.log(childPlant,"childPlantchildPlantgetCNGEnergy",HierarchyData,selecetedHierarchy)
                    }else{
                        viids = childPlant.node.nodes.filter(f=> f.type === 4)[0].nodes.map((valt) => valt.id);
                        viids.push(childPlant.node.nodes.filter(f=> f.type === 4)[0].asset) 
                        EnergyAsset = childPlant.node.nodes.filter(f=> f.type === 4)[0].asset
                    }
    ...
    return { CNGEnergyLoading, CNGEnergyData, CNGEnergyError, getCNGEnergy };
}


export default useCNGEnergy;