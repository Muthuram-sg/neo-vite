...
const useEnergyAllPlants = () => {
    const [EnergyAllPlantsLoading, setLoading] = useState(false);
    const [EnergyAllPlantsData, setData] = useState(null);
    const [EnergyAllPlantsError, setError] = useState(null);
    const [customdatesval] = useRecoilState(customdates);

    const getEnergyAllPlants  = async (childLine,btGroupValue,userDefaultList,typelist,headPlant ) => {
        setLoading(true);
        ...
        Promise.all(
            childLine.filter(e=> e.line_id !== headPlant.id).map(async (val,i) =>{
                ...
                if (childPlant.node && childPlant.node.nodes && (childPlant.node.nodes.filter(f=> f.type === 1).length>0)) {
                    let type = childPlant.node.nodes.filter(f=> f.type === 1)[0].radio
                    let HierarchyData = []
                    if(type === 'hierarchy'){
                    ...
                    if (type === 'hierarchy' && childPlant.node.nodes.filter(f=> f.type === 1)[0].selectedHierarchy && HierarchyData.length > 0) {
                        selecetedHierarchy = HierarchyData.find(x => x.id === childPlant.node.nodes.filter(f=> f.type === 1)[0].selectedHierarchy)
                        ...
                        viids = childPlant.node.nodes.filter(f=> f.type === 1)[0].nodes.map((valt) => valt.id);
                        viids.push(childPlant.node.nodes.filter(f=> f.type === 1)[0].asset) 
                        EnergyAsset = childPlant.node.nodes.filter(f=> f.type === 1)[0].asset
                    }
    ...
    return { EnergyAllPlantsLoading, EnergyAllPlantsData, EnergyAllPlantsError, getEnergyAllPlants };
}


export default useEnergyAllPlants;