...
const useLPGEnergy = () => {
    const [LPGEnergyLoading, setLoading] = useState(false);
    const [LPGEnergyData, setData] = useState(null);
    const [LPGEnergyError, setError] = useState(null);
    const [customdatesval] = useRecoilState(customdates);

    const getLPGEnergy  = async (childLine,btGroupValue,userDefaultList,typelist,headPlant ) => {
        setLoading(true);
        ...
        Promise.all(
            childLine.filter(e=> e.line_id !== headPlant.id).map(async (val,i) =>{
                ...
                if (childPlant.node && childPlant.node.nodes && (childPlant.node.nodes.filter(f=> f.type === 3).length>0)) {
                    let type = childPlant.node.nodes.filter(f=> f.type === 3)[0].radio
                    let HierarchyData = childPlant.hierarchies
                    if (type === 'hierarchy' && childPlant.node.nodes.filter(f=> f.type === 3)[0].selectedHierarchy && HierarchyData.length > 0) {
                        selecetedHierarchy = HierarchyData.find(x => x.id === childPlant.node.nodes.filter(f=> f.type === 3)[0].selectedHierarchy)
                        ...
                        viids = childPlant.node.nodes.filter(f=> f.type === 3)[0].nodes.map((valt) => valt.id);
                        viids.push(childPlant.node.nodes.filter(f=> f.type === 3)[0].asset) 
                        EnergyAsset = childPlant.node.nodes.filter(f=> f.type === 3)[0].asset
                    }
    ...
    return { LPGEnergyLoading, LPGEnergyData, LPGEnergyError, getLPGEnergy };
}


export default useLPGEnergy;