...
const useGetChannelListForContract= () => {
    ...
    const getChannelListForLine = async (line_id) => {
        ...
        await configParam.RUN_GQL_API(gqlQueries.getChannelListForLine, { line_id: line_id }) 
          .then((userLineData) => {
            ...
            if ( userLineData !== undefined && userLineData.neo_skeleton_notification_channels) {
                setData(userLineData.neo_skeleton_notification_channels.length > 0 ? userLineData.neo_skeleton_notification_channels : [])
                setError(false)
                setLoading(false)
            } else{
                setData(null)
...
export default useGetChannelListForContract;