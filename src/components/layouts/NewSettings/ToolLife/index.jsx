    ...
    const processedrows = () => {
        ...
        if (!ToolLifeLoading && ToolLifeData && !ToolLifeError) {
            ...
            temptabledata = temptabledata.concat(ToolLifeData.map((val, index) => { 
                return [index+1, val.name, val.asset_type.name, moment(val.updated_ts).format('DD/MM/YYYY HH:mm:ss') ,val.userByCreatedBy.name]
            }) 
            )
        } 
        setLoading(false)
                ...
                <Grid container spacing={3}>
                    <Grid item xs={12}>
                        <div className="h-[48px] py-3 px-4 border-b bg-Background-bg-primary dark:bg-Background-bg-primary-dark border-Border-border-50 dark:border-Border-border-dark-50">
                            <TypographyNDL value='Tool Life' variant='heading-02-xs'  />
                        </div>
