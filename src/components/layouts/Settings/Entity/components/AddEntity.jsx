      ...
      if (AddNewEntityData.affected_rows >= 1) {
        ...
        if ((Number(entityType.value) === 3 || Number(assetType.value) === 9) && instrument.value.length > 0) {
         ...
         if(Number(entityType.value) === 2 ){
          // console.log("enter",AddNewEntityData.returning[0].id,ZoneAsset)
        ...
        if (Number(entityType.value)=== 3 && oeeParams) {
          ...
          // getUpdatedEntityList({ variables: { line_id: headPlant.id }entityTyp= })
        }
        if(Number(entityType.value) === 3){
          handleFileUpload(AddNewEntityData.returning[0].id)
          return
...
function timeOutVal2(){
  ...
  const handleEditEnitytDialogOpen = (data) => {
   ...
   //  console.log("ImageURL before binding:", data.info.ImageURL); 
     setImageurl({ value:data.info.ImageURL , isValid: true })

      setTimeout(()=>{
     //   console.log("ImageURL after binding:", ImageURL.current.value);
    ...
    if (data.entity_instruments) {
     // console.log("entity_instruments:", data.entity_instruments);
      // eslint-disable-next-line array-callback-return
      data.entity_instruments.map((val) => {
      //  console.log("Matching instrument for instrument_id:", val.instrument_id);
    ...
    var oeeData = entity.filter(x => x.id === data.id)[0]
    var AnalyticConfig = props.AnalyticConfigListData.filter(x => x.entity_id === data.id)
...
// console.log("entitytype",entityType)
  ...
  configOeeDialog = () => {
    ...
    // if(assetType.value === 15 && ImageURL.current.value){
      ...
      if (entityType.value.toString() === "") {
        ...
        if (Number(entityType.value) === 3) {
              ...
              return false
                ...
                if (isReading && !isAnalytic) {
                  ...
                  if (entityDialogMode === "assetOEEparameter") {
                      ...
                      if (editCustomDash === true) {
                        ...
                        if (plannedSetupTimeRef.current) {
                          if (plannedSetupTimeRef.current.value === "") {
                            ...
                            if(isDryer){
                              setEntityDialogMode("assetOEEparameter4")
                              // console.log("assetOEEparameter4")
                              handleNext()
                            }else{
                              setEntityDialogMode("assetOEEparameter2")
                              // console.log("assetOEEparameter2")
                              handleNext()
                    ...
                    setEntityDialogMode('assetOEEparameter2') 
                    // console.log("assetOEEparameter22")
                    handleNext()
                    setTimeout(timeOutVal2, 500)
                  }
                  else  if  (entityDialogMode === "assetOEEparameter0"){
                    setEntityDialogMode("assetOEEparameter")
                    // console.log("assetOEEparameterr")
                    handleNext()
                  ...
                  setOeeParam(true)
                  // console.log(setOeeParam,"setOEEEParam")
                      ...
                      if (editCustomDash === true) {
                        ...
                        if (plannedSetupTimeRef.current) {
                          if (plannedSetupTimeRef.current.value === "") {
                            ...
                            if(isDryer){
                              setEntityDialogMode("assetOEEparameter4")  
                              // console.log("assetOEEparameter44")  
                              handleNext()
                            }else{
                  ...
                  if(entityDialogMode === "assetOEEparameter0"){
                    ...
                    if (updateedititem === true) {
                      getEditAnEntity(currEntityID, currUser.id, EntityNameRef.current ? EntityNameRef.current.value : entityName.value, entityType.value, assetType.value,{isFaultAnalysis:isFaultAnalysis,contractInstrument:contractTarget.value,target:contractTargetPercentage.current &&  contractTargetPercentage.current.value ? contractTargetPercentage.current.value : '',tenure:contractTenure.current && contractTenure.current.value ? contractTenure.current.value : ''})
                      getAnalyticConfig(currEntityID, config)
                      setBtnLoad(true)
                    } else {
                      setBtnLoad(true)
          ...
          if (entityDialogMode === "create") {
            getAddNewEntity(currUser.id, EntityNameRef.current ? EntityNameRef.current.value : entityName.value, entityType.value, assetType.value, headPlant.id,{isFaultAnalysis:isFaultAnalysis,contractInstrument:contractTarget.value,target:contractTargetPercentage.current &&  contractTargetPercentage.current.value ? contractTargetPercentage.current.value : '',tenure:contractTenure.current && contractTenure.current.value ? contractTenure.current.value : '',ImageURL:imageurl.value  ? imageurl.value   : '-'},isZone);
            setBtnLoad(true)
          }
          else if (entityDialogMode === "edit") {
            setBtnLoad(true)
...
const checkcontractTargetPercentage=(e)=>{
  if(e.target.value !== ''){
    if(Number(e.target.value) > 100){
      setiscontractTargetPercentage(true)
    }else{
    setiscontractTargetPercentage(false)
    }
  }else{
    setiscontractTargetPercentage(false)
  }
...
const handleRemoveAssetImage = ()=>{
 ...
 <Toast type={snType} message={snackMsg} toastBar={snackOpen}  handleSnackClose={() => setsnackOpen(false)} ></Toast>
  ...
  <SelectBox
              ...
              auto={false}
              multiple={false}
              options={!EntityTypeLoading && !EntityTypeError && EntityTypeData ? EntityTypeData : []}
              isMArray={true}
              checkbox={false}
...
/>            </div>
          ...
          <InputFieldNDL
          label={"% of Target"}
          inputRef={contractTargetPercentage}
          type={"number"}
          placeholder={t("Enter Target")}
          error={iscontractTargetPercentage}
          helperText={renderHelperText()}
          onChange={checkcontractTargetPercentage}
        />
            <div  className="mb-3" />
          ...
          {entityDialogMode !== "delete" && entityDialogMode !== "assetOEEparameter" && Number(entityType.value) === 3 && entityDialogMode !== "assetOEEparameter2" && entityDialogMode !== "assetOEEparameter3" && entityDialogMode !== "assetOEEparameter4" &&  entityDialogMode !== "assetOEEparameter0" &&
            <div style={{marginBottom:'12px'}}>
              <SelectBox
                ...
                // error={!instrument.isValid ? true : false}
                //msg={t('PlsSelInstru')}
              />
              <br></br>
              {
...
<br></br>
{
  ...
  <div className="mb-3" style={{display:'flex',justifyContent:'space-between'}}>
          ...
          <Grid container spacing={3} style={{alignItems:'center'}} >
              ...
              <Grid item xs={2}>
                ...
                {/* <CustomTextField
                  ...
                  label={t("Card Color")}
                  type="color"
                  inputRef={assetOoeAboveColorRef}
                  error={!oeeValueAbove.isValid ? true : false}
                  placeholder={t("Type value")}
                  disabled={true}
                /> */}
          ...
          <Grid container spacing={3} style={{alignItems:'center'}} >
            <Grid item xs={10}>
              {/* <DialogContentText>If OEE is between (in%)</DialogContentText> */}
              <InputFieldNDL
                id="OEE is between"
        ...
        />
              ...
              {/* <CustomTextField
                ...
                label={t("Card Color")}
                type="color"
                inputRef={assetOoeBetweenColorfnRef}
                error={!oeeValueBetween.isValid ? true : false}
                helperText={!oeeValueBetween.isValid ? t("Enter Value") : ""}
                placeholder={t("Type value")}
                disabled={true}
...

{
        ...
        <Grid item xs={12}>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <div className="mb-3">
              <TypographyNDL value={t('Metric Configuration')} variant='label-02s' />
              ...
              <Grid container spacing={3}>  
                ...
                <Grid item lg={6}>
                  <SelectBox
                    ...
                    // defaultDisableName="select instrument"
                    id="select-part-signal-instrument"
                    auto={true}
                    multiple={false}
                    options={instrument.value}
        ...
        <ModalFooterNDL>
          ...
          {/* <TertiaryButton onClick={() => { clickEntityButton() }} value={entityDialogMode === "create" ? t('Save') : entityDialogMode === "edit" ? t('Update') : t('YesDelete')} /> */}
