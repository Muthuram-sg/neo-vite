...
import useAddGroupMetric from "components/layouts/Settings/MetricsGroup/hooks/useAddGroupMetric.jsx"
import useDeleteGroupMetric from "components/layouts/Settings/MetricsGroup/hooks/useDeleteGroupMetric.jsx"
import useEditGroupMetric from "components/layouts/Settings/MetricsGroup/hooks/useEditGroupMetric.jsx"
...
export default function NewGateway(props) {
    ...
    useEffect(() => {
        ...
        setButtonDisabled(totalMetricsCount >= 11);
    ...
    const onClickHandler = () => { 
        ...
        if (
            pairs.length > 0 &&
            pairs.some(pair => pair.metrics.length === 0 || pair.metrics.some(metric => metric.id === '' || metric.id === null))
        ) {
            message += "Metric ID, ";
            isFieldMissing = true;
        }
        ...
        if (props.dialogMode === "edit") {
            getUpdateMetricsGroup(requestBody);
        } else if (props.dialogMode === "create") { 
            getCreateMetricsGroup(requestBody);
        }
    }
    };
    ...
    return (
        ...
        <ModalHeaderNDL>
            ...
            <TypographyNDL variant="heading-02-xs" model value={title} />
                {/* <TypographyNDL variant="heading-02-xs" model value={"Add Metric Group"} /> */}
               {props.dialogMode !== "delete" && <TypographyNDL variant="paragraph-xs" color="tertiary" value={t("Personalize your factory's identity, location, and business hierarchy ")} />}
            </div>
        </ModalHeaderNDL>
            <ModalContentNDL>
            {
                props.dialogMode === "delete" ? (
                    ...
                    <React.Fragment>
                        <TypographyNDL variant="paragraph-xs" color="secondary" value={`Do you really want to delete the group ${props.editValue.grpname}? This action cannot be undone.`} />
                    </React.Fragment>
                    )
                ) : 
                    <React.Fragment>
                        <Grid container spacing={3}>
                            ...
                            <SelectBox
                                ...
                                info={accessType === 1 ? "This metric group is set to public, visible to everyone in the line. Keep your insights open and accessible to everyone." :accessType === 2  ?  'This metric group is set to public, visible to everyone in the line. Keep your insights open and accessible to everyone.' : ''}
                            />
                            <div className="pt-1 pb-1">
                            {/* {accessType === 1 && (
                                <TypographyNDL variant='paragraph-xs' value={"This metric group is set to public, visible to everyone in the line. Keep your insights open and accessible to everyone."} />
                            )}

                            {accessType === 2 && (
                                <TypographyNDL variant='paragraph-xs' value={"This metric group is set to private, visible only to you. Keep your insights secure and accessible only to yourself."} />
                            )} */}
                        ...
                        <Button 
                            ...
                            style={{ margin: "10px",marginLeft: '518px' }}
                        ...
                        <div className="pt-5 pb-5">
                        <Checkboxs disabled={checkboxDisabled} checked={checked} 
