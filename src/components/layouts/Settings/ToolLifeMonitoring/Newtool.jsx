...
const Newtool = React.forwardRef((props, ref) => {  
    ...
    (
      {  
        handleToolDialogOpen: (data) => { 
          ...
          // LimitTimeRef.current.textMaskInputElement.state.previousConformedValue="00:10:00"
...
useEffect(() => {
    // getGetAddToolLife(headPlant.id)
...
useEffect(() => {
  if (!AddToolLifeLoading && !AddToolLifeError && AddToolLifeData) {
    if (AddToolLifeData.affected_rows > 0) {
      ...
      setSnackDesc('New tool added succesfully')
...
const handleInstrumentsListChange = (event) => {
  ...
  const handleToolDialogClose = () => {
    ...
    function saveToolLife(){
      // console.log(LimitTimeRef.current,"LimitTimeRef")
        if(!ToolNameRef.current.value){
        ...
        if(props.dialogMode === 'edit'){
          Toolarr = props.ToolList.filter(f=> f.name === ToolNameRef.current.value && f.id !== ToolID)
          Instarr = props.ToolList.filter(f=> f.intruments[0].id === IntID && f.id !== ToolID)
        }else{
          Toolarr = props.ToolList.filter(f=> f.name === ToolNameRef.current.value)
          Instarr = props.ToolList.filter(f=> f.intruments[0].id === IntID )
        } 
        if(!assetType.value){
        ...
        if(!LimitRef.current.value){
            ...
            if(LimitRef.current.value.toString().length > 12 || LimitRef.current.value < 0){
              setLimitval({value: "", isValid: false})
              return false
            }else{
        ...
        if(Toolarr.length>0){
          ...
          setSnackDesc(ToolNameRef.current.value+' already Exist')
        ...
        if(Instarr.length>0){
          setSnackMessage('Warning!')
          setSnackDesc('The Instrument selected already configured')
          setSnackType("warning")
          setOpenSnack(true)
          return false
        }
        ...
        if(props.dialogMode === 'create'){
          setLoading(true)
          getAddToolLife(ToolNameRef.current.value,assetType.value,instrument.value,LimitRef.current.value,currUser.id,headPlant.id, LimitDuration ? LimitDuration:null)
        }else{
          setLoading(true)
          getUpdateTool(ToolID,ToolNameRef.current.value,assetType.value,instrument.value,LimitRef.current.value,currUser.id,resetTime,LimitDuration ? LimitDuration:null)
        }  
    }
    ...
    let Title = (props.dialogMode === 'delete') ? 'Delete' : 'Update'
    return (
        <React.Fragment>
            <ModalHeaderNDL>
            ...
            <ModalContentNDL>
              {props.dialogMode === 'delete' ?
                <TypographyNDL variant="lable-01-s" color="secondary" value={<React.Fragment>{t('Do you really want to delete the Tool') + t('TheLine') + headPlant.name + t('NotReversible')} <br></br> <TypographyNDL value={"Note: Tool Life Dashboard data will be deleted permanently."}  variant={'paragraph-sans-s'} color="danger" /></React.Fragment>}  />
                :
                ...
                <Grid item xs={12}>
                  <InputFieldNDL
                    ...
                    error={!ToolName.isValid ? true : false}
                ...
                <Grid item xs={12}>
                  <SelectBox
                    ...
                    auto={true}
                    multiple={false}
                    options={props.AssetTypeData ? props.AssetTypeData : []}
                    isMArray={true}
                    checkbox={false}
                ...
                <Grid item xs={12}>
                  <InputFieldNDL
                    ...
                    type={'number'}
                    error={!Limitval.isValid ? true : false}
